...
LESSONS = [
    ...
    {"subject": "Spanish", "topic": "Past Tense Conjugations", "mode": "single"},
    {"subject": "Art", "topic": "Color Theory", "mode": "single"},
]

# Categorized feedback based on attention levels
SUGGESTIONS = {
    "high": [
        "Great pacing today!",
...
# Bonus: Subject-specific tips to make it ultra-realistic
SUBJECT_TIPS = {
    ...
    "Spanish": "Switch to a quick speaking or listening exercise."
}
...
def create_files(num_files=20):
    ...
    for _ in range(num_files):
        ...
        # Widened the range so we get some genuinely bad classes to test the 'low' logic
        avg_att = random.randint(20, 95)

        if lesson["mode"] == "single":
            blocks = [{"id": 0, "attention": avg_att}]
        else:
            blocks = [{"id": x, "attention": clamp(avg_att + random.randint(-15, 15))} for x in range(6)]
        ...
        # Pick 2-3 general suggestions from the correct tier
        selected_suggestions = random.sample(pool, random.randint(2, 3))

        # 50% chance to append a subject-specific tip if one exists
