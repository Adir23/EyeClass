...
def get_monthly_insights():
    # Returns static or AI analysis for the monthly report
    return jsonify({
...
def chat():
    ...
    if has_ai:
        try:
            response = model.generate_content(f"You are a helpful teaching assistant. User says: {user_msg}")
            return jsonify({"reply": response.text})
        except:
            return jsonify({"reply": "AI Error."})
    return jsonify({"reply": "AI is disabled. Please check app.py configuration."})
